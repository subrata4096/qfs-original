...
long stripeSize = 8u <<  20; //we will keep it same as chunksize in Kfstypes.h
...
void createAndWriteFile(string& fname, int& fd)
{
    ...
    //if ((fd = gKfsClient->Create(fname.c_str(),1,false,12,3,stripeSize,3)) < 0) {  //subrata: KFS_STRIPED_FILE_TYPE_RS_JERASURE = 3 // force use of Jerasure library
